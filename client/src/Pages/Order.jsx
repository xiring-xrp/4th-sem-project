...
function Order(){
    ...
    const [orderData, setOrderData] = useState({  
        clothingType: state.clothing_type,
        fabric: "",
        color: "",
        rate:"",
    });
    ...
    function rateGenerator() {
        ...
        const fab = state.fabrics.filter((item) => item.fabricId.fabric_name === fabric);
        setOrderData({
            ...orderData,
                ...
                <form noValidate className="flex flex-col">
                    ...
                    >
                        ...
                        {state.fabrics.map((fabric,index)=>{
                            return <option value={fabric.fabricId.fabric_name} className="text-black" key={index}>{fabric.fabricId.fabric_name}</option>
                        })
                        }
                    </select>
                    ...
                    >
                        ...
                        {state.colors.map((color,index)=>{
                            return <option value={color} className="text-black" key={index}>{color}</option>
                        })
                        }
                    </select>
                    ...
                    <button type="submit" className="mt-2 bg-yellow-600 hover:bg-yellow-500 transition-all ease-in-out duration-300 rounded-sm px-5 py-2 font-semibold text-lg cursor-pointer">CONFIRM ORDER</button>
